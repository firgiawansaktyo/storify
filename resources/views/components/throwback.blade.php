<div x-data class="container-storify bg-[var(--spotify-gray-bold)]">
    <h2 class="text-xl font-bold">Throwbacks</h2>
                ...
                <div class="carousel-card-throwback flex-shrink-0 w-48 h-64 p-1"
                    ...
                    <div class="throwback-cover p-1 rounded-lg cursor-pointer hover:bg-[var(--spotify-gray-semibold)] hover:scale-105 transition flex flex-col">
                        <div class="relative aspect-square overflow-hidden w-full rounded-lg">
                            <img
                            src="{{ asset('storage/' . $throwback->wedding_throwback_image) }}"
                            alt="image-{{ $throwback->wedding_throwback_title }}"
                            class="rounded-lg w-full h-full object-cover"
                            />
                        </div>
                        <div class="relative flex flex-col justify-center pt-2 h-15">
                            <span class="text-sm font-semibold truncate">{{ $throwback->wedding_throwback_title }}</span>
                            <p class="text-[var(--spotify-gray)] text-xs line-clamp-2">{{ $throwback->wedding_throwback_description }}</p>
                        </div>
                    </div>
                </div>
            @endforeach
